...
- **Dockerfile**: The deps stage runs `npm ci` (full install) so devDependencies like `cross-env` are available when `npm run build` runs in the builder stage. The final image still only contains the standalone output, not devDependencies.
